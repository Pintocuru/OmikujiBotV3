...
<template>
  ...
  <div class="space-y-4">
    ...
    >
      ...
      >
        <!-- ドラッグハンドル -->
        <div class="drag-handle cursor-move shrink-0">
          <GripVertical class="w-4 h-4 text-base-content/40 hover:text-base-content/80" />
        </div>

        <!-- 色インジケーター -->
        <div class="w-3 h-3 rounded-full shrink-0" :style="{ backgroundColor: getColorForIndex(index) }"></div>
...
<script setup lang="ts">
  ...
  import { CharacterType, generateId } from '@/types'
