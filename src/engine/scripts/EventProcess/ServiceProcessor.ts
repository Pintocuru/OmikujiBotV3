...
export class EventServiceProcessor {
  ...
  private async processMetaRules(event: MetaUpdateEvent): Promise<void> {
    const resolvedEvent: MetaUpdateEvent =
      event.previous === null && (this.lastUpVote !== null || this.lastViewer !== null)
        ? {
            ...event,
            previous: {
    ...
    const enabledEvents = Object.values(this.store.data.services)
      .filter((rule) => rule.isEnabled)
      .sort((a, b) => a.order - b.order)

    for (const event of enabledEvents) {
      if (evaluateServiceTrigger(event.trigger, resolvedEvent)) {
        const messages = await this.omikujiProcessor.executeOmikuji(event.key, event.omikuji, 'metas')
